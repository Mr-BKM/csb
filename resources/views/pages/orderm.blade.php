        ...
        @if (session('success'))
            ...
            @endif @if (session('error'))
                ...
                <div class="row">
                    <div class="col">
                        <!-- Order Form -->
                        <form class="row g-2" method="POST" action="{{ route('orderm.tempsaveData') }}">
                            @csrf
                            <div class="col-sm-7">
                                <div class="card">
                                    <div class="card-body">
                            ...
                            <div class="col-sm-5">
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <input type="hidden" name="order_typ" id="order_typ" value="Running">
                        ...
                        @if ($orderms->isEmpty())
                            <div class="row g-2">
                                <div class="col-md-9">
                                    <div class="card">
                                        <div class="card-body">
                                            {{-- <h5 class="card-title mb-1">Load Pending Order</h5> --}}
                                            {{-- <br> --}}
                                            <form class="row g-3" method="POST"
                                                action="{{ route('orderm.loadPendingOrder') }}">
                                                @csrf
                                                <input type="hidden" name="new_running_order_id"
                                                    value="{{ $neworderId }}">

                                                <div class="d-flex align-items-center gap-1">
                                                    <!-- Select -->
                                                    <select id="pending_order_slt" class="form-select w-auto flex-grow-1"
                                                        name="pending_order_slt" required>
                                                        <option value="" disabled selected>Pending Orders</option>
                                                    ...
                                                    <!-- Button -->
                                                    <button class="btn btn-dark" type="submit">Load Pending
                                                        Order</button>
                                                </div>
                                            </form>
                                ...
                                <div class="col-md-3">
                                    <div class="card">
                                        <div class="card-body">
                                            <form class="row g-3" method="POST"
                                                action="{{ route('orderm.loadreorderlevelorder') }}">
                                                @csrf
                                                    <!-- Button -->
                                                    <button class="btn btn-secondary" type="submit">Load Reorder Level Order</button>
                                ...
                                <div class="card-body">
                                    <div class="table-responsive table-centered">
                                        <table class="table text-nowrap mb-0">
                                            ...
                                            <tbody>
                                                @forelse ($orderms as $orderm)
                                                    <tr>
                                                        <td>{{ $loop->iteration }}</td>
                                                        <td>{{ $orderm->cus_name }}</td>
                                                        <td>{{ $orderm->itm_code }}</td>
                                                        <td>{{ $orderm->item->itm_name ?? '-' }}</td>
                                                        <td>{{ $orderm->item->itm_unit_of_measure ?? '-' }}</td>
                                                        <td>{{ $orderm->item->itm_stock ?? '-' }}</td>
                                                        <td>{{ $orderm->itm_qty }}</td>
                                                        <td>
                                                            <button type="button"
                                                                class="btn btn-sm btn-soft-secondary me-1"
                                                                data-bs-toggle="modal"
                                                                data-bs-target="#UpdateModalCenter{{ $orderm->id }}">
                                                                <i class="bx bx-edit fs-16"></i>
                                                            </button>
                                                            <button type="button" class="btn btn-sm btn-soft-danger"
                                                                data-bs-toggle="modal"
                                                                data-bs-target="#deleteModal{{ $orderm->id }}">
                                                            ...
                                                            <!-- Modal Delete-->
                                                            <div class="modal fade" id="deleteModal{{ $orderm->id }}"
                                                                tabindex="-1"
                                                                aria-labelledby="deleteModalLabel{{ $orderm->id }}"
                                                                aria-hidden="true">
                                                                <div class="modal-dialog" role="document">
                                                                    <div class="modal-content">
                                                                        ...
                                                                        <div
                                                                            ...
                                                                            <a href="{{ route('orderm.deleteData', $orderm->id) }}"
                        ...
                        @if ($orderms->isNotEmpty())
                            <div class="col-12 text-end mb-3 d-flex justify-content-end gap-3">
                                <!-- Pending Order Button (Trigger Modal) -->
                                <button type="button" class="btn btn-warning" data-bs-toggle="modal"
                                    data-bs-target="#pendingOrderModal{{ $neworderId }}">
                                    Pending Order
                                </button>

                                <!-- Pending Order Confirmation Modal -->
                                <div class="modal fade" id="pendingOrderModal{{ $neworderId }}" tabindex="-1"
                                    aria-labelledby="pendingOrderLabel{{ $neworderId }}" aria-hidden="true">
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            <!-- Modal Header -->
                                            <div class="modal-header">
                                                <h5 class="modal-title fw-bold text-warning"
                                                    id="pendingOrderLabel{{ $neworderId }}">
                                            ...
                                            <div class="modal-body text-center">
                                                <p class="mb-2 fw-semibold fs-5">⚠️ Are you sure you want to
                                                    mark this order as pending?</p>
                                                <p class="mb-0 text-muted">
                                                    Once confirmed, this order will be marked as <strong>Pending</strong>.
                                                </p>
                                            </div>
                                            ...
                                            <div class="modal-footer d-flex justify-content-end">
                                                ...
                                                <form action="{{ route('orderm.markPending') }}" method="POST"
                                                    class="d-inline">
                                                    @csrf
                                                    <input type="hidden" name="order_id" value="{{ $neworderId }}">
                                    ...
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer d-flex justify-content-end">
                                                ...
                                                <form action="{{ route('orderm.finishOrder') }}" method="POST"
                                                    class="d-inline">
                                                    @csrf
                                                    <input type="hidden" name="order_id" value="{{ $neworderId }}">
